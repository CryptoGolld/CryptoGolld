...
export async function createPoolWithLiquidity(config, keypair) {
...
async function resolveTokenInfo(sdk, coinType, explicitMetadataId) {
    ...
    if (metadataObjectId === undefined || metadataObjectId === null) {
        throw new Error(`Unable to resolve metadata object id for coin ${normalized}. Provide 'baseMetadataId' or 'quoteMetadataId' in the config or ensure the coin is registered in Cetus.`);
    }
    return {
        coinType: normalized,
        decimals,
...
function resolveTokenDecimals(token, metadata) {
...
function deriveQuoteAmount(baseAmount, price, quoteDecimals) {
...
async function ensureCoinBalance(client, owner, coinType, required) {
    ...
    if (available < needed) {
        throw new Error(`Insufficient balance for ${coinType}. Required ${needed.toString()} but only ${available.toString()} available.`);
    }
}
