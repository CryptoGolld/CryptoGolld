...
export function loadConfigFromFile(filePath) {
    ...
    for (const candidate of candidatePaths) {
        ...
        try {
            ...
            throw new Error(`Unable to parse config file at ${resolved}: ${error.message}`);
        }
    }
    throw new Error(`No configuration file found. Checked: ${candidatePaths
        .map((candidate) => path.resolve(process.cwd(), candidate))
        .join(', ')}`);
}
export function resolveConfig(rawConfig) {
    const { network, baseCoinType, baseAmount, initialPrice, lowerPrice, upperPrice, tickSpacing, } = rawConfig;
    if (!network) {
